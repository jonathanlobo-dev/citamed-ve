...
const specialtiesData = [
  ...
  },
  {
    ...
    commonConditions: ['Gastritis', 'Reflujo', '�alceras', 'Colitis', 'Hepatitis', 'Cirrosis'],
...
module.exports = {
  ...
  async seedSpecialties(Specialty) {
    console.log('�xR� Iniciando carga de especialidades médicas...');
    ...
    try {
      ...
      for (const specialty of specialtiesData) {
        ...
        if (created) {
          createdCount++;
          console.log(`  �S& ${specialty.name} creada`);
        } else {
          skippedCount++;
      ...
      console.log('\n�x` Resumen de carga:');
      console.log(`  �S& Especialidades creadas: ${createdCount}`);
      console.log(`  ⏭️  Especialidades omitidas: ${skippedCount}`);
      console.log(`  �x� Total en catálogo: ${specialtiesData.length}`);
      
      return { createdCount, skippedCount, total: specialtiesData.length };
      
    } catch (error) {
      console.error('�R Error cargando especialidades:', error);
      throw error;
    }
  }
};
