// src/models/index.js
// MÓDULO 2 - CITAMED.VE
// Índice de modelos con asociaciones corregidas
...
try {
  ...
  // IMPORTANTE: Solo pasar sequelize, no Sequelize
  const UserModel = require('./User');
  db.User = UserModel(sequelize);  // ✅ Solo un parámetro
  console.log('  ✅ User cargado');
  
  const SpecialtyModel = require('./Specialty');
  db.Specialty = SpecialtyModel(sequelize);  // ✅ Solo un parámetro
  console.log('  ✅ Specialty cargado');
  
  const DoctorProfileModel = require('./DoctorProfile');
  db.DoctorProfile = DoctorProfileModel(sequelize);  // ✅ Solo un parámetro
  console.log('  ✅ DoctorProfile cargado');
  
  const PatientProfileModel = require('./PatientProfile');
  db.PatientProfile = PatientProfileModel(sequelize);  // ✅ Solo un parámetro
  console.log('  ✅ PatientProfile cargado');
  
  const AppointmentModel = require('./Appointment');
  db.Appointment = AppointmentModel(sequelize);  // ✅ Solo un parámetro
